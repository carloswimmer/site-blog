...
export const Search = () => {
  ...
  const handleQueryChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (newQuery.trim()) {
      return router.push(`/blog?q=${encodeURIComponent(newQuery)}`, {
        scroll: false,
      });
    }
  ...
  return (
    <form onSubmit={handleSearch} className="relative group w-full md:w-60">
      ...
      {query && (
        <CircleX
          className="text-gray-300 absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4"
          onClick={resetSearch}
        />
      )}
    </form>
